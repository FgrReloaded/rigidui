...
function CompletionStep() {
    return <div className="flex flex-col items-center">
        <h2>Success!</h2>
        <span>Your data was successfully submitted</span>
    </div>
}
...
export function MultiStepFormWrapper<T extends FormData = FormData>({
    ...
    const CurrentStepComponent = isComplete ? completionStep : steps[currentStep];
    const { title, description, schema: stepSchema, onEnter, onExit } = (CurrentStepComponent?.props as StepProps<T>) || {}
    ...
    const goToNextStep = useCallback(async (): Promise<void> => {
        const validate = (CurrentStepComponent?.props as StepProps<T>)?.validate
        const stepSchema = (CurrentStepComponent?.props as StepProps<T>)?.schema
        const canSkip = (CurrentStepComponent?.props as StepProps<T>)?.canSkip || false
        ...
        if (stepSchema && !canSkip) {
            ...
            try {
                ...
                if (!result) {
                    ...
                    const errorMessage = (CurrentStepComponent?.props as StepProps<T>)?.validationMessage || 'Please fix the validation errors'
            ...
            try {
                ...
                if (!isValid) {
                    const errorMessage = (CurrentStepComponent?.props as StepProps<T>)?.validationMessage || 'Validation failed'
                    setStepErrors(prev => ({ ...prev, [currentStep]: errorMessage }))
                    return
                }
            } catch (error) {
    ...
    return (
        <div className={cn("max-w-2xl mx-auto", className)}>
            <MultiStepFormContext.Provider value={contextValue}>
                ...
                {navigationPosition === 'top' && !isComplete && renderNavigation()}
                ...
                {navigationPosition === 'bottom' && !isComplete && renderNavigation()}
