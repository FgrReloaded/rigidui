      ...
      <div className="container mx-auto px-4">
        ...
        <div className="space-y-4 md:space-y-3">
          {features.map((feature, index) => (
            <div
              key={index}
              className={`flex flex-col md:flex-row items-center gap-3 md:gap-4 p-6 md:py-4 md:px-8 rounded-2xl shadow-xl hover:shadow-2xl transition-all duration-300 ease-in-out transform hover:-translate-y-1 border border-gray-200 dark:border-gray-700/80 bg-white dark:bg-transparent backdrop-blur-xs ${index % 2 === 0 ? 'md:flex-row' : 'md:flex-row-reverse'
                }`}
            >
              <div className="shrink-0 rounded-full bg-gray-100 dark:bg-black w-20 h-20 md:w-8 md:h-8 flex items-center justify-center text-indigo-600 dark:text-indigo-400 ring-4 ring-indigo-500/20 dark:ring-indigo-500/30 mb-4 md:mb-0">
              ...
              <div className={`grow text-center ${index % 2 === 0 ? 'md:text-left' : 'md:text-right'}`}>
